...
class TestAvailabilityRule:
    def test_partial_overlap_rejects(self) -> None:
        ...
        assert AvailabilityRule[_Worker, _Request]().check(worker, allocation) is False
    ...
    def test_no_overlap_accepts(self) -> None:
        ...
        allocation = _request(1, 5)
        assert AvailabilityRule[_Worker, _Request]().check(worker, allocation) is True
...
class TestRequestedEntityRule:
    def test_empty_requested_entities_accepts_anyone(self) -> None:
        ...
        assert RequestedEntityRule[_Worker, _Request]().check(worker, allocation) is True
    ...
    def test_non_empty_requested_entities_restricts(self) -> None:
        ...
        assert rule.check(chosen, allocation) is True
    ...
    def test_identity_not_structural_equality(self) -> None:
        ...
        entity the caller put in the allocation", so identity is the
        relevant relation.
        """
        chosen = _Worker(name="A", unavailabilities=[])
        ...
        rule = RequestedEntityRule[_Worker, _Request]()
        assert rule.check(chosen, allocation) is True
        assert rule.check(lookalike, allocation) is False
...
class TestLoadBalancingAssignmentAlgorithm:
    def test_picks_highest_scored_candidate(self) -> None:
        ...
        allocation = _request(1, 2)
        candidates = {
            id(allocation): [Candidate(entity=low, score=0.3), Candidate(entity=high, score=0.9)],
        }
...
class TestConsoleOutputAdapter:
    def test_prints_each_assignment(self, capsys: object) -> None:
        worker = _Worker(name="W", unavailabilities=[])
        allocation = _request(1, 2)
        state: AssignmentState[_Worker, _Request] = AssignmentState()
        state.add_assignment(Assignment(allocation=allocation, assigned_entities=(worker,)))
