...
def test_assigns_simple_problem() -> None:
    ...
    assert result.assignments[0].assigned_entities == (worker,)
...
def test_skips_allocation_with_insufficient_candidates() -> None:
    ...
    a = _Worker(name="A", unavailabilities=[])
    allocation = _request(1, 2, required_count=3)
    candidates = {id(allocation): [Candidate(entity=a)]}
...
def test_skips_candidate_whose_entity_is_not_in_entities_list() -> None:
    ...
    allocation = _request(1, 2)
    candidates = {id(allocation): [Candidate(entity=orphan, score=0.9), Candidate(entity=in_list, score=0.5)]}
    ...
    assert len(result.assignments) == 1
    assert result.assignments[0].assigned_entities == (in_list,)
...
def test_allocation_with_no_eligible_candidates_is_skipped() -> None:
    ...
    assert len(result.assignments) == 1
    assert result.assignments[0].allocation is fillable
...
def test_tiny_score_does_not_collapse_to_empty_assignment() -> None:
    ...
    worker = _Worker(name="solo", unavailabilities=[])
    allocation = _request(1, 2)
    candidates = {id(allocation): [Candidate(entity=worker, score=0.0001)]}
...
def test_zero_score_remains_zero() -> None:
    ...
    worker = _Worker(name="solo", unavailabilities=[])
    allocation = _request(1, 2)
    candidates = {id(allocation): [Candidate(entity=worker, score=0.0)]}
