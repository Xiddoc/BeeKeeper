...
`examples/mcdonalds/` ships three fixture sizes generated by `examples/mcdonalds/scripts/generate_large_fixtures.py` (deterministic for a fixed seed):
...
`tests/benchmarks/test_algorithm_benchmarks.py` runs each algorithm × each fixture (parametrized). Two budgets: warning at 500 ms (emits a `UserWarning`, doesn't fail), hard ceiling at 1 s (fails). On a developer laptop the four algorithms land roughly:

| | large (65/25) | xlarge (100/40) | xxlarge (200/80) |
| --- | --- | --- | --- |
| greedy | ~8 ms | ~19 ms | ~65 ms |
| backtracking | ~8 ms | ~18 ms | ~67 ms |
| load_balancing | ~9 ms | ~20 ms | ~75 ms |
| or_tools | ~60 ms | ~510 ms (warn) | ~670 ms (warn) |

OR-Tools is order-of-magnitude slower because of its model-build + solver-init costs. The 500 ms warning on xlarge/xxlarge is expected.
